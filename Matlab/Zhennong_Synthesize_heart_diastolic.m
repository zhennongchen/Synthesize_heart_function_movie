...
diastolic_lag = 0.5;
disp(['diastolic lag equal to ',num2str(diastolic_lag)]);

makemovie = 1;

if makemovie == 1
    save_path = ['/Volumes/McVeighLab/projects/Zhennong/Video_Prediction/Synthesized_EF_movie_2/',p_class,'_',p_id,'_',num2str(round(info.ef_desired,2)),'_',num2str(round(diastolic_lag,2)),'%'];
    ...
    time_frames_num = size([1:2:info.tf]);
    time_frames_num = time_frames_num(2);
    
    systole_interval = 2; diastole_interval = systole_interval * diastolic_lag;
    dia_time_frame = [];
    for kkk = 1:time_frames_num
        ttt = round(info.tf - diastole_interval * (kkk-1));
        dia_time_frame = [dia_time_frame ttt];
    end
        
    for i = dia_time_frame
        clf;
